...
export default class Crudeoperation extends React.Component<ICrudeoperationProps, crudeStates> {
  private dataTableRef;
    ...
    public onSave(){
      ...
      this.setState({toUsers:namearr,modalOpen:false});
    }

    public onToCloseModal(){
      debugger;
      this.setState({
        modalOpen:false
      });
    ...
    public render(): React.ReactElement<ICrudeoperationProps> {
      ...
      return (
        ...
        <button className='toempid' onClick={(e)=>this.onToOpenModal()} style={{background:'none',backgroundColor:'white',border:'none',padding:'5px',color:'orange',display:'inline-block'}}><i className="fa fa-plus" aria-hidden="true" style={{border:'1px',backgroundColor:'#2a2a2c',color:'white'}}></i>Open</button>
          
          <Modal open={this.state.modalOpen} onClose={(e)=>this.onToCloseModal()} center>
            <PeoplePicker    
                ...
                resolveDelay={1000} /> 
                <div>
                  <button type="button" onClick={(e)=>this.onSave()}>Save</button>
                </div>
          </Modal>
          </div>
          </div>
